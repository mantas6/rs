// Item definitions. Data-only: plain objects, no logic.
//
// Equipment requirements deviate slightly from OSRS for early-game pacing:
// bronze tier is unrestricted, iron tier requires attack 5 and steel tier
// requires attack 10 (so requirement gating is exercised from the very start
// of the game). Gathering tools follow the same pacing: bronze tools are
// usable at level 1, iron tools require level 5 and steel tools require
// level 10 in the matching gather skill.
import type { ItemDef } from './types'

export const items: Record<string, ItemDef> = {
  coins: {
    id: 'coins',
    name: 'Coins',
    examine: 'Lovely money!',
    stackable: true,
    value: 1,
  },
  bronze_axe: {
    id: 'bronze_axe',
    name: 'Bronze axe',
    examine: 'A woodcutting axe.',
    stackable: false,
    value: 16,
    tool: { kind: 'axe', tier: 1, requiredLevel: 1 },
    equipment: {
      slot: 'weapon',
      bonuses: { attackSlash: 3, attackCrush: 1, meleeStrength: 2 },
      attackSpeed: 4,
      attackType: 'slash',
    },
  },
  iron_axe: {
    id: 'iron_axe',
    name: 'Iron axe',
    examine: 'A woodcutting axe.',
    stackable: false,
    value: 56,
    tool: { kind: 'axe', tier: 2, requiredLevel: 5 },
    equipment: {
      slot: 'weapon',
      requirements: { attack: 5 },
      bonuses: { attackSlash: 5, attackCrush: 2, meleeStrength: 4 },
      attackSpeed: 4,
      attackType: 'slash',
    },
  },
  steel_axe: {
    id: 'steel_axe',
    name: 'Steel axe',
    examine: 'A woodcutting axe.',
    stackable: false,
    value: 200,
    tool: { kind: 'axe', tier: 3, requiredLevel: 10 },
    equipment: {
      slot: 'weapon',
      requirements: { attack: 10 },
      bonuses: { attackSlash: 7, attackCrush: 3, meleeStrength: 6 },
      attackSpeed: 4,
      attackType: 'slash',
    },
  },
  bronze_pickaxe: {
    id: 'bronze_pickaxe',
    name: 'Bronze pickaxe',
    examine: 'Used for mining.',
    stackable: false,
    value: 1,
    tool: { kind: 'pickaxe', tier: 1, requiredLevel: 1 },
    equipment: {
      slot: 'weapon',
      bonuses: { attackStab: 4, attackCrush: 2, meleeStrength: 3 },
      attackSpeed: 5,
      attackType: 'stab',
    },
  },
  iron_pickaxe: {
    id: 'iron_pickaxe',
    name: 'Iron pickaxe',
    examine: 'Used for mining.',
    stackable: false,
    value: 140,
    tool: { kind: 'pickaxe', tier: 2, requiredLevel: 5 },
    equipment: {
      slot: 'weapon',
      requirements: { attack: 5 },
      bonuses: { attackStab: 6, attackCrush: 3, meleeStrength: 4 },
      attackSpeed: 5,
      attackType: 'stab',
    },
  },
  steel_pickaxe: {
    id: 'steel_pickaxe',
    name: 'Steel pickaxe',
    examine: 'Used for mining.',
    stackable: false,
    value: 500,
    tool: { kind: 'pickaxe', tier: 3, requiredLevel: 10 },
    equipment: {
      slot: 'weapon',
      requirements: { attack: 10 },
      bonuses: { attackStab: 8, attackCrush: 4, meleeStrength: 6 },
      attackSpeed: 5,
      attackType: 'stab',
    },
  },
  small_fishing_net: {
    id: 'small_fishing_net',
    name: 'Small fishing net',
    examine: 'Useful for catching small fish.',
    stackable: false,
    value: 5,
    tool: { kind: 'net', tier: 1, requiredLevel: 1 },
  },
  tinderbox: {
    id: 'tinderbox',
    name: 'Tinderbox',
    examine: 'Useful for lighting a fire.',
    stackable: false,
    value: 1,
  },
  bronze_sword: {
    id: 'bronze_sword',
    name: 'Bronze sword',
    examine: 'A razor sharp sword.',
    stackable: false,
    value: 26,
    equipment: {
      slot: 'weapon',
      bonuses: { attackStab: 4, attackSlash: 3, defenceStab: 2, meleeStrength: 5 },
      attackSpeed: 5,
      attackType: 'stab',
    },
  },
  bronze_scimitar: {
    id: 'bronze_scimitar',
    name: 'Bronze scimitar',
    examine: 'A vicious, curved sword.',
    stackable: false,
    value: 32,
    equipment: {
      slot: 'weapon',
      bonuses: { attackSlash: 7, attackStab: 1, meleeStrength: 6 },
      attackSpeed: 4,
      attackType: 'slash',
    },
  },
  bronze_2h_sword: {
    id: 'bronze_2h_sword',
    name: 'Bronze 2h sword',
    examine: 'A sword requiring two hands to wield.',
    stackable: false,
    value: 84,
    equipment: {
      slot: 'weapon',
      twoHanded: true,
      bonuses: { attackSlash: 7, attackCrush: 6, attackStab: -4, meleeStrength: 10 },
      attackSpeed: 7,
      attackType: 'slash',
    },
  },
  wooden_shield: {
    id: 'wooden_shield',
    name: 'Wooden shield',
    examine: 'A solid wooden shield.',
    stackable: false,
    value: 20,
    equipment: {
      slot: 'shield',
      bonuses: {
        defenceStab: 4,
        defenceSlash: 5,
        defenceCrush: 3,
        defenceMagic: -1,
        defenceRanged: 4,
      },
    },
  },
  bronze_full_helm: {
    id: 'bronze_full_helm',
    name: 'Bronze full helm',
    examine: 'A full face helmet.',
    stackable: false,
    value: 44,
    equipment: {
      slot: 'head',
      bonuses: {
        defenceStab: 4,
        defenceSlash: 5,
        defenceCrush: 3,
        defenceMagic: -1,
        defenceRanged: 4,
        attackMagic: -6,
        attackRanged: -2,
      },
    },
  },
  bronze_platebody: {
    id: 'bronze_platebody',
    name: 'Bronze platebody',
    examine: 'Provides excellent protection.',
    stackable: false,
    value: 160,
    equipment: {
      slot: 'body',
      bonuses: {
        defenceStab: 15,
        defenceSlash: 14,
        defenceCrush: 9,
        defenceMagic: -6,
        defenceRanged: 14,
        attackMagic: -30,
        attackRanged: -10,
      },
    },
  },
  bronze_platelegs: {
    id: 'bronze_platelegs',
    name: 'Bronze platelegs',
    examine: 'These look pretty heavy.',
    stackable: false,
    value: 80,
    equipment: {
      slot: 'legs',
      bonuses: {
        defenceStab: 8,
        defenceSlash: 7,
        defenceCrush: 5,
        defenceMagic: -4,
        defenceRanged: 7,
        attackMagic: -21,
        attackRanged: -7,
      },
    },
  },
  logs: {
    id: 'logs',
    name: 'Logs',
    examine: 'A number of wooden logs.',
    stackable: false,
    value: 4,
  },
  oak_logs: {
    id: 'oak_logs',
    name: 'Oak logs',
    examine: 'Logs cut from an oak tree.',
    stackable: false,
    value: 20,
  },
  copper_ore: {
    id: 'copper_ore',
    name: 'Copper ore',
    examine: 'This needs refining.',
    stackable: false,
    value: 3,
  },
  tin_ore: {
    id: 'tin_ore',
    name: 'Tin ore',
    examine: 'This needs refining.',
    stackable: false,
    value: 3,
  },
  iron_ore: {
    id: 'iron_ore',
    name: 'Iron ore',
    examine: 'This needs refining.',
    stackable: false,
    value: 17,
  },
  steel_ore: {
    id: 'steel_ore',
    name: 'Steel ore',
    examine: 'This needs refining.',
    stackable: false,
    value: 34,
  },
  bronze_bar: {
    id: 'bronze_bar',
    name: 'Bronze bar',
    examine: "It's a bar of bronze.",
    stackable: false,
    value: 8,
  },
  iron_bar: {
    id: 'iron_bar',
    name: 'Iron bar',
    examine: "It's a bar of iron.",
    stackable: false,
    value: 17,
  },
  steel_bar: {
    id: 'steel_bar',
    name: 'Steel bar',
    examine: "It's a bar of steel.",
    stackable: false,
    value: 34,
  },
  raw_shrimps: {
    id: 'raw_shrimps',
    name: 'Raw shrimps',
    examine: 'I should try cooking these.',
    stackable: false,
    value: 5,
  },
  shrimps: {
    id: 'shrimps',
    name: 'Shrimps',
    examine: 'Some nicely cooked shrimps.',
    stackable: false,
    value: 5,
    healAmount: 3,
  },
  burnt_shrimps: {
    id: 'burnt_shrimps',
    name: 'Burnt shrimps',
    examine: 'Oops!',
    stackable: false,
    value: 1,
  },
  raw_trout: {
    id: 'raw_trout',
    name: 'Raw trout',
    examine: 'I should try cooking this.',
    stackable: false,
    value: 20,
  },
  trout: {
    id: 'trout',
    name: 'Trout',
    examine: 'Some nicely cooked trout.',
    stackable: false,
    value: 20,
    healAmount: 7,
  },
  burnt_trout: {
    id: 'burnt_trout',
    name: 'Burnt trout',
    examine: 'Oops!',
    stackable: false,
    value: 1,
  },
  bones: {
    id: 'bones',
    name: 'Bones',
    examine: 'Bones are for burying!',
    stackable: false,
    value: 1,
    buryXp: 4.5,
  },
  cowhide: {
    id: 'cowhide',
    name: 'Cowhide',
    examine: 'I should take this to the tannery.',
    stackable: false,
    value: 1,
  },
  raw_beef: {
    id: 'raw_beef',
    name: 'Raw beef',
    examine: 'I should try cooking this.',
    stackable: false,
    value: 1,
  },
  cooked_beef: {
    id: 'cooked_beef',
    name: 'Cooked beef',
    examine: 'Mmm, this looks tasty.',
    stackable: false,
    value: 4,
    healAmount: 3,
  },
  burnt_beef: {
    id: 'burnt_beef',
    name: 'Burnt beef',
    examine: 'Oops!',
    stackable: false,
    value: 1,
  },
  raw_chicken: {
    id: 'raw_chicken',
    name: 'Raw chicken',
    examine: 'I need to cook this before eating it.',
    stackable: false,
    value: 1,
  },
  cooked_chicken: {
    id: 'cooked_chicken',
    name: 'Cooked chicken',
    examine: 'Mmm, this looks tasty.',
    stackable: false,
    value: 4,
    healAmount: 3,
  },
  burnt_chicken: {
    id: 'burnt_chicken',
    name: 'Burnt chicken',
    examine: 'Oops!',
    stackable: false,
    value: 1,
  },
  feather: {
    id: 'feather',
    name: 'Feather',
    examine: 'Used for fly fishing.',
    stackable: true,
    value: 2,
  },
  leather: {
    id: 'leather',
    name: 'Leather',
    examine: 'A piece of tanned cowhide.',
    stackable: false,
    value: 20,
  },
  needle: {
    id: 'needle',
    name: 'Needle',
    examine: 'Used with thread to make leather items.',
    stackable: false,
    value: 1,
  },
  thread: {
    id: 'thread',
    name: 'Thread',
    examine: 'Used to make things.',
    stackable: true,
    value: 1,
  },
  leather_gloves: {
    id: 'leather_gloves',
    name: 'Leather gloves',
    examine: 'These will keep my hands warm!',
    stackable: false,
    value: 6,
    equipment: {
      slot: 'hands',
      bonuses: { defenceStab: 1, defenceSlash: 1, defenceCrush: 1 },
    },
  },
  leather_boots: {
    id: 'leather_boots',
    name: 'Leather boots',
    examine: 'Comfortable leather boots.',
    stackable: false,
    value: 6,
    equipment: {
      slot: 'feet',
      bonuses: { defenceStab: 1, defenceSlash: 1, defenceCrush: 1 },
    },
  },
  leather_body: {
    id: 'leather_body',
    name: 'Leather body',
    examine: 'It provides some protection.',
    stackable: false,
    value: 25,
    equipment: {
      slot: 'body',
      bonuses: { defenceStab: 4, defenceSlash: 4, defenceCrush: 4, defenceRanged: 4 },
    },
  },
  leather_chaps: {
    id: 'leather_chaps',
    name: 'Leather chaps',
    examine: 'Fine leather chaps.',
    stackable: false,
    value: 20,
    equipment: {
      slot: 'legs',
      bonuses: { defenceStab: 2, defenceSlash: 2, defenceCrush: 2, defenceRanged: 2 },
    },
  },
  knife: {
    id: 'knife',
    name: 'Knife',
    examine: 'A sharp knife, useful for fletching.',
    stackable: false,
    value: 6,
  },
  arrow_shafts: {
    id: 'arrow_shafts',
    name: 'Arrow shafts',
    examine: 'Some carefully cut arrow shafts.',
    stackable: true,
    value: 1,
  },
  shortbow_u: {
    id: 'shortbow_u',
    name: 'Shortbow (u)',
    examine: 'An unstrung shortbow; it needs a string.',
    stackable: false,
    value: 5,
  },
  longbow_u: {
    id: 'longbow_u',
    name: 'Longbow (u)',
    examine: 'An unstrung longbow; it needs a string.',
    stackable: false,
    value: 10,
  },
  oak_shortbow_u: {
    id: 'oak_shortbow_u',
    name: 'Oak shortbow (u)',
    examine: 'An unstrung oak shortbow; it needs a string.',
    stackable: false,
    value: 20,
  },
  bowstring: {
    id: 'bowstring',
    name: 'Bow string',
    examine: 'Used to string bows.',
    stackable: true,
    value: 2,
  },
  shortbow: {
    id: 'shortbow',
    name: 'Shortbow',
    examine: 'A quick, short-range bow. Needs arrows.',
    stackable: false,
    value: 50,
    equipment: {
      slot: 'weapon',
      twoHanded: true,
      rangedWeapon: true,
      requirements: { ranged: 1 },
      bonuses: { attackRanged: 8 },
      attackSpeed: 4,
      attackRange: 7,
    },
  },
  longbow: {
    id: 'longbow',
    name: 'Longbow',
    examine: 'A slower bow with greater range. Needs arrows.',
    stackable: false,
    value: 80,
    equipment: {
      slot: 'weapon',
      twoHanded: true,
      rangedWeapon: true,
      requirements: { ranged: 1 },
      bonuses: { attackRanged: 10 },
      attackSpeed: 6,
      attackRange: 9,
    },
  },
  oak_shortbow: {
    id: 'oak_shortbow',
    name: 'Oak shortbow',
    examine: 'A sturdy oak shortbow. Needs arrows.',
    stackable: false,
    value: 120,
    equipment: {
      slot: 'weapon',
      twoHanded: true,
      rangedWeapon: true,
      requirements: { ranged: 5 },
      bonuses: { attackRanged: 14 },
      attackSpeed: 4,
      attackRange: 7,
    },
  },
  bronze_arrowtips: {
    id: 'bronze_arrowtips',
    name: 'Bronze arrowtips',
    examine: 'Sharp bronze arrow heads.',
    stackable: true,
    value: 1,
  },
  headless_arrow: {
    id: 'headless_arrow',
    name: 'Headless arrow',
    examine: 'An arrow shaft with feathers but no head.',
    stackable: true,
    value: 1,
  },
  bronze_arrow: {
    id: 'bronze_arrow',
    name: 'Bronze arrow',
    examine: 'Arrows with bronze heads.',
    stackable: true,
    value: 1,
    equipment: {
      slot: 'ammo',
      requirements: { ranged: 1 },
      bonuses: { rangedStrength: 7 },
    },
  },
  potato_seed: {
    id: 'potato_seed',
    name: 'Potato seed',
    examine: 'A seed. Plant it in an allotment patch.',
    stackable: true,
    value: 1,
  },
  onion_seed: {
    id: 'onion_seed',
    name: 'Onion seed',
    examine: 'A seed. Plant it in an allotment patch.',
    stackable: true,
    value: 5,
  },
  cabbage_seed: {
    id: 'cabbage_seed',
    name: 'Cabbage seed',
    examine: 'A seed. Plant it in an allotment patch.',
    stackable: true,
    value: 7,
  },
  potato: {
    id: 'potato',
    name: 'Potato',
    examine: 'A freshly dug potato.',
    stackable: false,
    value: 1,
    healAmount: 1,
  },
  onion: {
    id: 'onion',
    name: 'Onion',
    examine: 'It is making my eyes water.',
    stackable: false,
    value: 2,
    healAmount: 1,
  },
  cabbage: {
    id: 'cabbage',
    name: 'Cabbage',
    examine: 'A leafy green cabbage.',
    stackable: false,
    value: 2,
    healAmount: 2,
  },
  beer: {
    id: 'beer',
    name: 'Beer',
    examine: 'A glass of frothy ale.',
    stackable: false,
    value: 2,
    drink: {
      healAmount: 1,
      boosts: [
        { skill: 'strength', delta: 2 },
        { skill: 'attack', delta: -3 },
      ],
      emptyItemId: 'beer_glass',
    },
  },
  beer_glass: {
    id: 'beer_glass',
    name: 'Beer glass',
    examine: 'The dregs of a fine ale. A true bokalas.',
    stackable: false,
    value: 1,
  },
  grimy_guam: {
    id: 'grimy_guam',
    name: 'Grimy guam leaf',
    examine: 'I need to clean this herb before using it.',
    stackable: false,
    value: 5,
  },
  guam_leaf: {
    id: 'guam_leaf',
    name: 'Guam leaf',
    examine: 'A cleaned herb, ready for a vial of water.',
    stackable: false,
    value: 8,
  },
  grimy_tarromin: {
    id: 'grimy_tarromin',
    name: 'Grimy tarromin',
    examine: 'I need to clean this herb before using it.',
    stackable: false,
    value: 12,
  },
  tarromin: {
    id: 'tarromin',
    name: 'Tarromin',
    examine: 'A cleaned herb, ready for a vial of water.',
    stackable: false,
    value: 18,
  },
  vial_of_water: {
    id: 'vial_of_water',
    name: 'Vial of water',
    examine: 'A vial filled with water.',
    stackable: false,
    value: 2,
  },
  empty_vial: {
    id: 'empty_vial',
    name: 'Vial',
    examine: 'An empty glass vial.',
    stackable: false,
    value: 1,
  },
  eye_of_newt: {
    id: 'eye_of_newt',
    name: 'Eye of newt',
    examine: 'An eyeball of some description.',
    stackable: false,
    value: 5,
  },
  limpwurt_root: {
    id: 'limpwurt_root',
    name: 'Limpwurt root',
    examine: 'A root from a limpwurt plant.',
    stackable: false,
    value: 15,
  },
  guam_potion_unf: {
    id: 'guam_potion_unf',
    name: 'Guam potion (unf)',
    examine: 'I need another ingredient to finish this potion.',
    stackable: false,
    value: 10,
  },
  tarromin_potion_unf: {
    id: 'tarromin_potion_unf',
    name: 'Tarromin potion (unf)',
    examine: 'I need another ingredient to finish this potion.',
    stackable: false,
    value: 20,
  },
  attack_potion: {
    id: 'attack_potion',
    name: 'Attack potion',
    examine: 'A potion that temporarily boosts your Attack.',
    stackable: false,
    value: 12,
    drink: {
      boosts: [{ skill: 'attack', delta: 3 }],
      emptyItemId: 'empty_vial',
    },
  },
  strength_potion: {
    id: 'strength_potion',
    name: 'Strength potion',
    examine: 'A potion that temporarily boosts your Strength.',
    stackable: false,
    value: 24,
    drink: {
      boosts: [{ skill: 'strength', delta: 3 }],
      emptyItemId: 'empty_vial',
    },
  },
}
